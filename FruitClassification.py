...
data_dir='D:/final_year_project1/imageclassify'
#print(os.listdir(os.path.join(data_dir,'/content/drive/MyDrive/images4classify/humans')))
img_exts=['jpg','png','jpeg','bmp']

#img=cv2.imread(os.path.join('image4classify','human','human3bodies2.jpg'))
#type(img)
#img.shape
#plt.imshow(cv2.cvtColor(img,cv2.COLOR_BGR2RGB))
#img.show()
#print(os.path.exists('C:/Users/dell/PycharmProjects/pythonProject/nimage4classify/banana/image4.jpeg'))
...
data=tf.keras.utils.image_dataset_from_directory('imageclassify')#building data pipeline
...
def submit():
  ...
  name = im.get()
  im.set("")
  img=cv2.imread('D:/final_year_project1/'
+name+".jpg")
  plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
  ...
  if  yhat >0.5:
    print(f'Predicted class is banana')
    result = tkinter.Label(frame, text='Predicted class is apple', font=('calibre', 10, 'bold'))
    result.pack()
  else:
    print(f'Predicted class is apple')
    result = tkinter.Label(frame, text='Predicted class is banana', font=('calibre', 10, 'bold'))
    result.pack()
